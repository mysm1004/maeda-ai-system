...
function sendLine(message) {
  ...
  if (user && user.user_id) {
    return pushLine(user.user_id, msg);
  }
  return new Promise(function(resolve) {
...
async function runSleepMode() {
  ...
  if (activeSessions.length === 0) {
    console.log('[就寝モード] アクティブなセッションなし');
    return;
  }
...
async function sendMorningSummary() {
  ...
  // v2.0: 朝サマリーはClaude Opusで要約
  var r = await anthropic.messages.create({
    model: 'claude-opus-4-6', max_tokens: 1500,
    system: '就寝中の議論サマリーを簡潔に作成してください。重要ポイント・提案・決定事項を箇条書きで。1-2行でまとめること。',
    messages: [{ role: 'user', content: logText.substring(0, 8000) }]
  });
...
async function sendWeeklyReport() {
  ...
  var r = await anthropic.messages.create({
    ...
    system: '週次レポートを1-2行でまとめてください。成果・来週の推奨アクション。',
    messages: [{ role: 'user', content: 'セッション: ' + sessions.length + '件\n承認済み: ' + cases.length + '件\n休眠プロジェクト: ' + sleeping.length + '件' }]
  });
