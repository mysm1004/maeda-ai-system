// fix_extract.js
// 1. extractHTMLFromContent の正規表現修正（閉じ```なし対応）
// 2. 既存HTMLを再生成
// 3. LINE通知
...
// ============================================
// Fix 1: server.js の extractHTMLFromContent を修正
// ============================================
var svFile = '/home/ubuntu/kabeuchi-system/src/server.js';
var sv = fs.readFileSync(svFile, 'utf8');

var oldExtract = "function extractHTMLFromContent(raw) {\n" +
  "  var m = raw.match(/```html\\s*\\n([\\s\\S]*?)```/);\n" +
  "  if (m) return m[1].trim();";

var newExtract = "function extractHTMLFromContent(raw) {\n" +
...
if (sv.indexOf(oldExtract) >= 0) {
  sv = sv.replace(oldExtract, newExtract);
  fs.writeFileSync(svFile, sv, 'utf8');
  console.log('[Fix 1] extractHTMLFromContent 修正完了');
} else {
  console.log('[Fix 1] SKIP - パターン不一致');
}

// ============================================
// Fix 2: 既存output_queueからHTML再生成
// ============================================
...
oldFiles.forEach(function(f) { fs.unlinkSync(path.join(outputDir, f)); });
console.log('[Cleanup] ' + oldFiles.length + '件の古いHTMLを削除');

var typeLabels = {lp:'LP', proposal:'提案書', dm:'DM', sales_script:'営業スクリプト', blog:'ブログ', seminar:'セミナー資料'};
...
rows.forEach(function(row) {
  ...
  patterns.forEach(function(p, i) {
    ...
    if (!html) {
      // HTMLが抽出できない場合はテキストをHTML化
      html = '<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>' + (typeLabels[row.output_type] || '') + '</title>' +
  ...
  console.log('[HTML] ' + indexName);
  allResults.push({ sessionId: row.session_id, title: row.title, typeLabel: typeLabel, indexFile: indexName });
});

console.log('\n====== 完了: ' + allResults.length + '件 ======');
allResults.forEach(function(r) {
  console.log('ID:' + r.sessionId + ' ' + r.title + ' ' + r.typeLabel);
  console.log('  https://176-32-87-118.sslip.io/outputs/' + r.indexFile);
});
